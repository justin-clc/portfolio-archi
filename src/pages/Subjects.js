...
import SemesterDropdown from "../components/SemeterDropdown";
...
export default function Subjects() {
	...
	const btnSelected =
		"mb-3 me-3 rounded-full border border-accent bg-primary px-5 py-2.5 text-center text-base font-medium text-accent transition-colors hover:text-white focus:outline-none";
	...
	return (
		<main className="-mt-16 w-full bg-bgMedium">
			<section className="bg-Medium bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] pt-32">
				<div className="lg:pt-18 relative z-10 mx-auto max-w-screen-xl px-4 py-8 text-center lg:pb-12">
					{/* <a
          ...
          >
            ...
            >
              ...
              />
					...
					<h1 className="mt- mb-4 font-primary text-4xl font-extrabold leading-none tracking-tight text-white md:text-5xl lg:text-6xl">
						{genContent.SchoolYears?.heading + " " + year}
					</h1>
					<p className="mb-8 font-secondary text-lg font-normal text-bgLight sm:px-16 lg:px-48 lg:text-xl">
				...
				{semester1.length === 0 && semester2.length === 0 ? (
					<h3 className="m-auto mt-8 max-w-5xl text-center font-primary text-3xl font-bold text-white md:text-4xl">
						No Subjects Yet
					</h3>
				) : (
					<>
						{selectedSemester === "1" && semester1.length > 0 && (
							<Semester
								SemesterType={1}
								SemesterArray={semester1}
							/>
						)}
						{selectedSemester === "2" && semester2.length > 0 && (
							<Semester
								SemesterType={2}
								SemesterArray={semester2}
							/>
						)}
					</>
				)}
